; vim: set expandtab:smarttab
; {{ serial_comment }}
@ 1H IN SOA ns0.wikimedia.org. hostmaster.wikimedia.org. {{ serial_num }} 12H 2H 2W 1H

; Name servers

@           1D  IN NS   ns0.wikimedia.org.
@           1D  IN NS   ns1.wikimedia.org.
@           1D  IN NS   ns2.wikimedia.org.

; Mail exchangers

@           300 IN MX   10      mx-in1001.wikimedia.org.
@           300 IN MX   10      mx-in2001.wikimedia.org.


; Canonical names
@           180 IN DYNA geoip!text-addrs

; Google Webmaster Tools verification record for search management
@           600     IN TXT  "google-site-verification=8Twc3OrOQ9oBq5C81ZCsPa3fRRJT1xdt5yDBvdnDtUQ"
; Permissions for additional owners added on Aug 25. See noc email.
@           600     IN TXT  "google-site-verification=nOKTUn6gWZ44XDh-0H0euRlNJtxCyTP5K8Mb66ty7Bs"

; SPF records
@           600     IN TXT  "v=spf1 include:wikimedia.org ~all"

; DMARC record
_dmarc      600     IN TXT "v=DMARC1; p=reject; rua=mailto:dmarc-rua@wikimedia.org;"

; CAA records
@           600 IN CAA 0 issue "letsencrypt.org"
@           600 IN CAA 0 issue "pki.goog"
@           600 IN CAA 0 iodef "mailto:dns-admin@wikimedia.org"

; Servers (alphabetic order)

; Service aliases
query                        1D IN CNAME dyna.wikimedia.org.
query-next                   1D IN CNAME dyna.wikimedia.org.
query-main                   1D IN CNAME dyna.wikimedia.org.
query-scholarly              1D IN CNAME dyna.wikimedia.org.
query-scholarly-next         1D IN CNAME dyna.wikimedia.org.

; Wikis (alphabetic order)

test        1D IN CNAME dyna.wikimedia.org.
www         1D IN CNAME dyna.wikimedia.org.

; Mobile
m           1D IN CNAME dyna.wikimedia.org.
test.m      1D IN CNAME dyna.wikimedia.org.
