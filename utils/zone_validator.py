...
if __name__ == '__main__':
    gen_zones_module = importlib.import_module('gen-zones')
else:
    gen_zones_module = importlib.import_module('..gen-zones', package=__name__)
...
class ZonesValidator:
    ...
    def _parse(self):
        ...
        # The parsed files are generated by gen-zones.py that adds a header. Offset that out when reporting line
        # numbers so that they match the ones in the actual git repository.
        header_offset = -1 * len(gen_zones_module.HEADER.splitlines()) + 1
